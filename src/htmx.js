var htmx = (function() {
  ...
  function makeFragment(response) {
    ...
    if (startTag === 'html') {
      ...
      const part = parseHTML(response)
      takeChildrenFor(fragment, part.body)
      fragment.title = part.title
    } else if (startTag === 'body') {
      // parse body w/o wrapping in template
      fragment = /** @type DocumentFragmentWithTitle */ (new DocumentFragment())
      const part = parseHTML(responseWithNoHead)
      takeChildrenFor(fragment, part.body)
      fragment.title = part.title
    } else {
      // otherwise we have non-body partial HTML content, so wrap it in a template to maximize parsing flexibility
      const part = parseHTML('<body><template class="internal-htmx-wrapper">' + responseWithNoHead + '</template></body>')
      fragment = /** @type DocumentFragmentWithTitle */ (part.querySelector('template').content)
  ...
  function bodyContains(elt) {
    return elt.getRootNode({ composed: true }) === doc
  }
  ...
  function internalEval(str) {
    return maybeEval(doc.body, function() {
      return eval(str)
    })
  }
  ...
  function querySelectorExt(eltOrSelector, selector) {
    if (typeof eltOrSelector !== 'string') {
      ...
      return querySelectorAllExt(doc.body, eltOrSelector)[0]
    }
  }
  ...
  function resolveTarget(eltOrSelector, context) {
    if (typeof eltOrSelector === 'string') {
      return find(asParentNode(context) || doc, eltOrSelector)
    } else {
  ...
  function oobSwap(oobValue, oobElement, settleInfo, rootNode) {
    ...
    if (targets.length) {
      ...
      oobElement.parentNode.removeChild(oobElement)
      triggerErrorEvent(doc.body, 'htmx:oobErrorNoTarget', { content: oobElement })
    }
    return oobValue
  }
    ...
    forEach(findAll(fragment, '[hx-preserve], [data-hx-preserve]'), function(preservedElt) {
      ...
      if (existingElement != null) {
        if (preservedElt.moveBefore) { // if the moveBefore API exists, use it
          ...
          if (pantry == null) {
            doc.body.insertAdjacentHTML('afterend', "<div id='--htmx-preserve-pantry--'></div>")
        ...
        if (token === ']') {
          ...
          if (bracketCount === 0) {
            ...
            try {
              ...
              triggerErrorEvent(doc.body, 'htmx:syntax:error', { error: e, source: conditionalSource })
              return null
            }
          }
        } else if (token === '[') {
  ...
  function maybeFilterEvent(triggerSpec, elt, evt) {
    ...
    if (eventFilter) {
      try {
        ...
        triggerErrorEvent(doc.body, 'htmx:eventFilter:error', { error: e, source })
        return true
      }
    }
    return false
  ...
  function saveToHistoryCache(url, rootElt) {
    ...
    triggerEvent(doc.body, 'htmx:historyItemCreated', { item: newHistoryItem, cache: historyCache })
    ...
    while (historyCache.length > 0) {
      try {
        ...
        triggerErrorEvent(doc.body, 'htmx:historyCacheError', { cause: e, cache: historyCache })
  ...
  function saveCurrentPageToHistory() {
    ...
    if (!disableHistoryCache) {
      triggerEvent(doc.body, 'htmx:beforeHistorySave', { path, historyElt: elt })
  ...
  function loadHistoryFromServer(path) {
    ...
    request.onload = function() {
      if (this.status >= 200 && this.status < 400) {
        ...
        triggerEvent(doc.body, 'htmx:historyCacheMissLoad', details)
        swap(details.historyElt, details.response, swapSpec, {
          contextElement: details.historyElt,
          historyRequest: true
        })
        setCurrentPathForHistory(details.path)
        triggerEvent(doc.body, 'htmx:historyRestore', { path, cacheMiss: true, serverResponse: details.response })
      } else {
        triggerErrorEvent(doc.body, 'htmx:historyCacheMissLoadError', details)
      }
    }
    if (triggerEvent(doc.body, 'htmx:historyCacheMiss', details)) {
  ...
  function restoreHistory(path) {
    ...
    if (cached) {
      ...
      if (triggerEvent(doc.body, 'htmx:historyCacheHit', details)) {
        swap(details.historyElt, cached.content, swapSpec, {
          contextElement: details.historyElt,
          title: cached.title
        })
        setCurrentPathForHistory(details.path)
        triggerEvent(doc.body, 'htmx:historyRestore', details)
      }
    } else {
 ...
 */
  ...
  function getPathFromResponse(xhr) {
    if (xhr.responseURL) {
      try {
        ...
        triggerErrorEvent(doc.body, 'htmx:badResponseUrl', { url: xhr.responseURL })
      }
    }
  }
    ...
    if (beforeSwapDetails.shouldSwap) {
      ...
      swap(target, serverResponse, swapSpec, {
        ...
        beforeSwapCallback: function() {
          ...
          if (historyUpdate.type) {
            triggerEvent(doc.body, 'htmx:beforeHistoryUpdate', mergeObjects({ history: historyUpdate }, responseInfo))
            if (historyUpdate.type === 'push') {
              pushUrlIntoHistory(historyUpdate.path)
              triggerEvent(doc.body, 'htmx:pushedIntoHistory', { path: historyUpdate.path })
            } else {
              replaceUrlInHistory(historyUpdate.path)
              triggerEvent(doc.body, 'htmx:replacedInHistory', { path: historyUpdate.path })
            }
          }
        }
      })
  ...
  ready(function() {
    ...
    const originalPopstate = win.onpopstate ? win.onpopstate.bind(window) : null
    /** @type {(ev: PopStateEvent) => any} */
