...
* `htmx.config.historyRestoreAsHxRequest:false` -  Whether to treat history cache miss full page relaod requests as a "HX-Request" by returning this response header. This should be left disabled when using HX-Request header to optionally return partial responses
