...
As mentioned previously when using swap strategies other than `true` or `outerHTML` the encapsulating tags are stripped by default, however you still need to encapsulate the returned data with the correct tags for the content so it can be parsed as valid html.
