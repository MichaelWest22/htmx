...
const routes = {
    ...
    '/events': (req, res) => {
        ...
        res.write(`data: <htmx-partial hx-target="#events-output" hx-swap="beforeend"><div class="events-active"></div></htmx-partial>\n\n`);

        // Send initial status
        res.write(`data: <htmx-partial hx-target="#system-status" hx-swap="innerHTML">${status}</htmx-partial>\n\n`);
        ...
        const send = () => {
            ...
            res.write(`data: <htmx-partial hx-target="#activity" hx-swap="beforeend"><div>${activities[Math.floor(Math.random() * activities.length)]}</div></htmx-partial>\n\n`);
