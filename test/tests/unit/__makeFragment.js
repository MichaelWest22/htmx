describe('__makeFragment unit tests', function() {
    ...
    it('creates fragment from simple HTML', function () {
        let {fragment} = htmx.__makeFragment({text: '<div>Test</div>'});
    ...
    it('creates fragment from multiple elements', function () {
        let {fragment} = htmx.__makeFragment({text: '<div>A</div><span>B</span>'});
    ...
    it('extracts title from HTML document', function () {
        let {title} = htmx.__makeFragment({text: '<html><head><title>Test Title</title></head><body></body></html>'});
        title.should.equal('Test Title');
    })

    it('handles html tag', function () {
        let {fragment} = htmx.__makeFragment({text: '<html><body><div>Test</div></body></html>'});
        fragment.children[0].tagName.should.equal('DIV');
    })

    it('handles body tag', function () {
        let {fragment} = htmx.__makeFragment({text: '<body><div>Test</div></body>'});
        fragment.children[0].tagName.should.equal('DIV');
    })

    it('converts partial tags to template', function () {
        let {fragment} = htmx.__makeFragment({text: '<hx-partial hx-target="#foo">Content</hx-partial>'});
    ...
    it('strips head content', function () {
        let {fragment} = htmx.__makeFragment({text: '<head><title>Title</title></head><div>Body</div>'});
        assert.isNull(fragment.querySelector('head'));
        fragment.children[0].tagName.should.equal('DIV');
    })

    it('handles table row content', function () {
        let {fragment} = htmx.__makeFragment({text: '<tr><td>Cell 1</td><td>Cell 2</td></tr>'});
    ...
    it('handles multiple table rows', function () {
        let {fragment} = htmx.__makeFragment({text: '<tr><td>A</td></tr><tr><td>B</td></tr>'});
        fragment.children.length.should.equal(2);
        fragment.children[0].tagName.should.equal('TR');
    ...
    it('handles table cell content', function () {
        let {fragment} = htmx.__makeFragment({text: '<td>Cell Content</td>'});
    ...
    it('handles thead content', function () {
        let {fragment} = htmx.__makeFragment({text: '<thead><tr><th>Header</th></tr></thead>'});
    ...
    it('handles tbody content', function () {
        let {fragment} = htmx.__makeFragment({text: '<tbody><tr><td>Data</td></tr></tbody>'});
    ...
    it('handles mixed table elements', function () {
        let {fragment} = htmx.__makeFragment({text: '<thead><tr><th>H</th></tr></thead><tbody><tr><td>D</td></tr></tbody>'});
        fragment.children.length.should.equal(2);
        fragment.children[0].tagName.should.equal('THEAD');
