describe('swap() unit tests', function() {
    ...
    it('swaps partial with default target', async function () {
        await htmx.swap({"target":"#test-playground", "text":"<htmx-partial hx-target='#test-playground'>Partial</partial>"})
        playground().innerText.should.equal("Partial");
    })

    it('swaps partial with custom target', async function () {
        createProcessedHTML("<div id='d1'></div><div id='d2'></div>")
        await htmx.swap({"target":"#d1", "text":"<htmx-partial hx-target='#d2'>Partial</partial>"})
        findElt('#d2').innerText.should.equal("Partial");
    })

    it('swaps partial with custom swap style', async function () {
        createProcessedHTML("<div id='d1'>Existing</div>")
        await htmx.swap({"target":"#test-playground", "text":"<htmx-partial hx-target='#d1' hx-swap='beforeend'>Partial</partial>"})
    ...
    it('executes script in partial', async function () {
        window.testVar = 0;
        await htmx.swap({"target":"#test-playground", "text":"<htmx-partial hx-target='#test-playground'><script>window.testVar = 8</script></partial>"})
    ...
    it('sets title with partial swap', async function () {
        let originalTitle = document.title;
        createProcessedHTML("<div id='d1'></div>")
        await htmx.swap({"target":"#test-playground", "text":"<title>Partial Title</title><htmx-partial hx-target='#d1'>Partial Content</partial>"})
